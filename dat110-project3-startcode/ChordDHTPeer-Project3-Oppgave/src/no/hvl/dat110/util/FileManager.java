...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		for(int i = 0; i < numReplicas; i++) {
			file = filename + i;
			BigInteger bigHash = Hash.hashOf(file);
			replicafiles[i] = bigHash;
		}
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	createReplicaFiles();
		Random r = new Random();
		int primaryIndex = r.nextInt(replicafiles.length - 1);
    	
    	
    	for(BigInteger replicaFile : replicafiles) {
    		NodeInterface succFileID = chordnode.findSuccessor(replicaFile);
    		
    		if(primaryIndex == counter) {
    			chordnode.getFilesMetadata(succFileID.getNodeID()).setPrimaryServer(true);
    		} else {
    			chordnode.getFilesMetadata(succFileID.getNodeID()).setPrimaryServer(false);
    		}
    				
    		boolean primary = chordnode.getFilesMetadata(succFileID.getNodeID()).isPrimaryServer();
    				
    		chordnode.getFilesMetadata(succFileID.getNodeID()).setPrimaryServer(true);
 
    		
    		if(succFileID != null) {
    			succFileID.addKey(replicaFile);
    			String saveContent = chordnode.getNodeName() + "\n" + chordnode.getNodeID();
    			succFileID.saveFileContent(saveContent, succFileID.getNodeID(), bytesOfFile, primary); //finne primary?
    			counter++;
    		}
    	}
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for(BigInteger replicaFile : replicafiles) {
			NodeInterface succFileID = chordnode.findSuccessor(replicaFile);
			if (succFileID != null) {
				Message message = succFileID.getFilesMetadata(replicaFile);
	...
	public NodeInterface findPrimaryOfItem() {
		...
		for(Message activeNodeMessage : activeNodesforFile) {
			if(activeNodeMessage.isPrimaryServer()) {
				NodeInterface fileNode = Util.getProcessStub(activeNodeMessage.getNodeIP(), activeNodeMessage.getPort());
				return fileNode;
			}
		}
