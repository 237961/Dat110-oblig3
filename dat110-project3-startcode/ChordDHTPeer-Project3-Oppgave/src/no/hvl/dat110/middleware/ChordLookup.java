...
public class ChordLookup {
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for(int i=0; i<fingerTable.size()-1; i++) {
			int m = size-i;
			NodeInterface FTpred = fingerTable.get(m);
			try {
				BigInteger FTpredID = FTpred.getNodeID();
				NodeInterface nodeStub = Util.getProcessStub(FTpred.getNodeName(), FTpred.getPort());
				if(Util.computeLogic(FTpredID, node.getNodeID().add(new BigInteger("1")), key.subtract(new BigInteger("1")))) {
					return nodeStub;
				}
			} catch (Exception e) {
