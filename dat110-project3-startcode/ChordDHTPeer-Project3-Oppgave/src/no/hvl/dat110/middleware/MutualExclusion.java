...
public class MutualExclusion {
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for(Message activenodeMsg : activenodes) {
			NodeInterface activenode = Util.getProcessStub(activenodeMsg.getNodeIP(), activenodeMsg.getPort());
			activenode.onMutexRequestReceived(message);
		}
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 0: {
				NodeInterface msgNode = Util.getProcessStub(procName, port);
				message.setAcknowledged(true);
				msgNode.onMutexAcknowledgementReceived(message);
	...
	public void multicastReleaseLocks(Set<Message> activenodes) {
		...
		for(Message activenode : activenodes) {
			NodeInterface msgNode = Util.getProcessStub(activenode.getNodeIP(), activenode.getPort());
			try {
				msgNode.releaseLocks();
